module calculations
    ...
    contains
        ...
        subroutine calculate_forces(particles, index_start, index_end, head, epsilon, theta, rank)
            type(particle3d), intent(inout) :: particles(:)
            type(cell), pointer :: head
            integer :: i
            real(real64), intent(in) :: epsilon, theta
            integer, intent(in) :: rank, index_start, index_end
            
            do i = index_start, index_end
                call calculate_forces_aux(particles, i, head, epsilon, theta, rank)
            end do
        end subroutine calculate_forces

        recursive subroutine calculate_forces_aux(particles, goal, tree, epsilon, theta, rank)
            type(particle3d), intent(inout) :: particles(:)
                        ...
                        do j = 1,2
                            do k = 1,2
                                if (associated(tree%subcell(i,j,k)%ptr)) then
                                    call calculate_forces_aux(particles, goal, tree%subcell(i,j,k)%ptr, epsilon, theta, rank)
                                end if
                            end do
                        end do
                    end do
