...
contains
  ...
  subroutine velocity(n, dt, p)
    ...
    !$omp parallel default(none) private(i) shared(n, dt, p)
    !$omp do schedule(static, 64) nowait
    do i = 1, n
  ...
  subroutine position(n, dt, p) !Subroutine to update the position of particles over time
    integer(int64), intent(in) :: n !Number of particles
    real(real64), intent(in) :: dt  !Time step
    type(particle3d), intent(inout) :: p(:) !Particles
  ...
  subroutine set_acceleration(n, p)
    ...
    !$omp parallel default(none) private(i) shared(n, p)
    !$omp do schedule(static, 64) nowait
    do i = 1, n
