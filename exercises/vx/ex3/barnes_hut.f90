...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE Calculate_forces(head, n, p, rji, a, i_start, i_end, rank)
    ...
    integer, intent(in) :: i_start, i_end, rank
    DO i = i_start, i_end
       CALL Calculate_forces_aux(i,head, p, rji, a, rank)
    END DO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  RECURSIVE SUBROUTINE Calculate_forces_aux(goal,tree, p, rji, a, rank)
             ...
             DO j = 1,2
                DO k = 1,2
                   IF (ASSOCIATED(tree%subcell(i,j,k)%ptr)) THEN
                      CALL Calculate_forces_aux(goal,tree%subcell(i,j,k)%ptr, p, rji, a, rank)
                   END IF
                END DO
             END DO
          END DO
